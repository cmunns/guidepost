---
title: Options
description: Every option accepted by new Tour(options), its default, and the lifecycle callbacks.
---

| Option | Default | What it does |
| --- | --- | --- |
| `steps` | — | Required. |
| `id` | — | Used by `remember` and written into the root as a data attribute. |
| `blocking` | `true` | Apply `inert` to the rest of the page. Also hides that content from assistive technology. |
...
| `fallbackPlacements` | `[]` | Tried in order when the preferred one does not fit (JS strategy). |
...
| `exitOnOverlayClick` | `false` | Clicking the dim cancels. Requires `blocking`. |
...
| `injectStyles` | `true` | Set false if you import the CSS yourself. |
| `container` | `document.body` | Where the tour DOM is appended. |
...
`labels` takes a `Partial<TourLabels>` — override only the strings you need.
